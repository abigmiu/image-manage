<template>
    <NCard>
        <NSpace>
            ...
            <NButton type="primary" dashed size="small" @click="onToggleCreate">
                <template #icon>
                    <NIcon>
                        <PlusOutlined />
                    </NIcon>
    ...
    <NModal v-model:show="showCreate" preset="card" style="width: 400px;">
        <NForm>
            <NFormItem label="名称">
                <NInput v-model:value="createValue.name"></NInput>
            </NFormItem>
            <NFormItem label="颜色">
                <NSelect v-model:value="createValue.colorType" :options="tagsColorOptions"></NSelect>
            </NFormItem>
            <NFormItem>
                <div class="flex flex-justify-end w-full">
                    <NButton @click="onToggleCreate" class="mr-2">取消</NButton>
                    <NButton @click="onSubmitCreateTag" :loading="createLoading" type="primary">确认</NButton>
                </div>
            </NFormItem>
        </NForm>
    </NModal>
